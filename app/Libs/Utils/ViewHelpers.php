...
class ViewHelpers
{
    ...
    // abbreviate a string (render as preview)
    public static function previewText(string $strIn, int $maxChars=150) : string
    {
        if( strlen($strIn) > $maxChars) {
            $a = explode( "\n", wordwrap( $strIn, $maxChars));
            $strOut = count($a) ? $a[0] . '...' : '--';
        } else {
            $strOut = $strIn;
        }
